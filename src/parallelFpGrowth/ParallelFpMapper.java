	...
	public void map(Object key,Text value,Context context) throws IOException,InterruptedException{
		...
		if(noOfGroups >= totalElements) {
			for(int i=0;i<totalElements;i++) {
				...
				//System.out.println(fList.get(i).getItem() + " -m- " + (i+1));
			}
		}
		else {
		...
		for(int j = size-1;j >= 0;j--) {
			...
			//System.out.println("item " + newTransactionList.get(j) + " gid " + gid);
			if(!visited.containsKey(gid)) {
				...
				// if (j==size-1){  //Debug
				// 	if (dependentTransactions.toString().matches("(.*)85(.*)")) { //debug use
				//    	 	if (dependentTransactions.toString().matches("(.*)90(.*)")) {
				// 		    if (dependentTransactions.toString().matches("(.*)93(.*)")) {
				// 				System.out.println(" -- " + newKey + " -- "+newValue);
				// 			}
				// 		}
				// 	}
				// }
				//System.out.println(dependentTransactions.toString() + " : " + newValue);
			}
